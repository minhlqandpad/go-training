...
func (db *MySQLDB) GetTenantByID(ctx context.Context, id string) (*domain.Tenant, error) {
    var tenant domain.Tenant
    query := "SELECT id, name, email FROM tenants WHERE id = ?"
    err := db.DB.QueryRowContext(ctx, query, id).Scan(&tenant.ID, &tenant.Name, &tenant.Email)
